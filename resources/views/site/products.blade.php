            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcroumb-title text-center">
                        ...
                        <h6><a href="{{ route('home') }}">{{ trans('site.home') }}</a> / <a href="">{{ app()->getLocale() == 'ar' ? $category->title_ar : $category->title_en }}</a>
                        </h6>
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        $('.CategorySort').on('click', function () {
            ...
            $.ajax({
                ...
                error: function (data) {
                    $('.productSearch').html('<h2 class="error">{{ app()->getLocale() == 'ar' ? 'لا يوجد منتجات' : 'NO PROJECTS FOUND' }}</h2>');
                }
            });
        });
            ...
            $('a#filter_project').on('click', function(event) {
                ...
                $.ajax({
                    ...
                    error: function (data) {
                        $('.productSearch').html('<h2 class="error">{{ app()->getLocale() == 'ar' ? 'لا يوجد مشاريع' : 'NO PROJECTS FOUND' }}</h2>');
                    }
                });
            });
        });
